    ...
    public function up()
    {
        if(!Schema::hasTable('kaltura_media')) {
            Schema::create('kaltura_media', function (Blueprint $table) {
                ...
                $table->string('license_type', 64)->comment("What kind of license has this video class LicenseType");
                $table->string('type')->nullable(false)->comment("What type of media entry is this. class EntryType");
                ...
                $table->text('tags')->nulleble(true)->comment("Tags that have been added to the video");
                ...
                $table->string('rootEntryId', 255)->comment("what is the original id of the entry");
                $table->timestamps();
            });
        }
    }
